...
let parseTokens = (lexer: lexer) => {
        ...
        | PExito(exprFinal) => {
            ...
            let rec funDesicion = (lexerRes, aceptarSoloOp, fnEnOp, funValorDefecto) => {
                ...
                | Token(token, _) => {
                    ...
                    | TIdentificador(_) | TNumero(_) | TTexto(_) | TBool(_) | TParenAb(_) when !aceptarSoloOp => {
    ...
    and sigExprIdentificador = (exprIdInfo: exprIdInfo, nivel, precedencia, _, esExprPrincipal) => {
        ...
        let rec funDesicion = (lexerRes, aceptarSoloOperador, fnEnOp, funValorDefecto) => {
            ...
            | Token (token, _) => {
                switch token {
                | TIdentificador(_) | TNumero(_) | TTexto(_) | TBool(_) | TParenAb(_) when !aceptarSoloOperador => {
                    lexer.retroceder();
